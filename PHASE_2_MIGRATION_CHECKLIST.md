...
- [ ] Exercise directories:
  ...
  - `exercises/ex006_sequence_modify_casting/` — legacy root-level exercise directory that duplicates the `ex006` identity.
  ...
  - `exercises/sequence/debug/ex007_sequence_debug_casting/` — old construct/type/exercise shape and affected by the ex007 naming mismatch.
...
- [ ] Notebook self-check contracts:
  ...
  - `notebooks/ex007_sequence_debug_casting.ipynb` currently calls `check_notebook('ex007_data_types_debug_casting')`, which is inconsistent with the student-checker API and the file name.
...
- [ ] Blocker: The current repository contains duplicate `ex006_sequence_modify_casting` exercise directories at `exercises/ex006_sequence_modify_casting/` and `exercises/sequence/modify/ex006_sequence_modify_casting/`, so exercise identity is ambiguous.
