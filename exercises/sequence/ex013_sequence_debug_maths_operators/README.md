# Debug Maths Operators

## Student prompt
- Open `notebooks/student.ipynb` in this exercise folder.
- Write your solution in the notebook cell tagged `exercise1` (or `exercise2`, …).
- From the repository root, run `PYTUTOR_ACTIVE_VARIANT=student uv run pytest -q exercises/sequence/ex013_sequence_debug_maths_operators/tests/test_ex013_sequence_debug_maths_operators.py` until all tests pass.
- After running your corrected solution, describe what happened in the cell tagged `explanation1` (or `explanationN`).

## Teacher notes
- Created: 2026-06-05
- Target concepts: (fill in)
