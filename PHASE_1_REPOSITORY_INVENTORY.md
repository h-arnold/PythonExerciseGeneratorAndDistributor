...
| `scripts/build_autograde_payload.py` | Notebook-path plus environment override | `_normalise_notebooks_dir()` and `validate_environment()` only allow `notebooks` and `notebooks/solutions` | Later variant selection must replace `legacy notebook-root override env var` as the layout contract |
...
| `template_repo_files/.github/workflows/classroom.yml` | Exported Classroom workflow assumes a flattened `notebooks/` tree and `legacy notebook-root override env var=notebooks` | Confirms the current exported student contract is metadata-free and flattened; later packaging changes must preserve that contract deliberately |
