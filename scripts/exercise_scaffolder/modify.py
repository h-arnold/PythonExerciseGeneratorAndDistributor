"""ModifyScaffold — exercise type scaffold for modify exercises.

Each part produces a markdown prompt + a tagged code cell with a TODO
placeholder for the student to write their solution.

Note that ModifyScaffold and MakeScaffold currently share the same cell
structure.  They are kept as separate classes intentionally — the semantics
of "modify existing code" vs "make something from scratch" may diverge in
...
class ModifyScaffold(ExerciseScaffold):
    """Scaffold for modify-type exercises.

    Students receive a prompt and write their solution in the tagged code cell.
    """
    ...
    def _build_exercise_body_test_lines(self) -> list[str]:
        """Return test assertions for modify exercise body cells.
