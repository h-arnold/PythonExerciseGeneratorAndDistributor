...
def test_iter_exercise_ids_for_framework_detailed_role() -> None:
    ...
    assert list(iter_exercise_ids_for_role(SupportRole.FRAMEWORK_DETAILED)) == [2]
...
def test_iter_exercise_ids_for_framework_smoke_role() -> None:
    ...
    assert list(iter_exercise_ids_for_role(SupportRole.FRAMEWORK_SMOKE)) == [3, 4, 5, 6]


def test_iter_exercise_ids_for_student_checker_role() -> None:
    """The support matrix exposes all student-checker-supported exercise IDs."""
    assert list(iter_exercise_ids_for_role(SupportRole.STUDENT_CHECKER)) == [2, 3, 4, 5, 6, 7]
...
def test_has_support_role_returns_false_for_unknown_exercise() -> None:
    ...
    assert not has_support_role(999, SupportRole.STUDENT_CHECKER)
