...
- Cell metadata must include `{"language": "python", "tags": ["exerciseN"]}`. The grading helper `runtime.run_cell_and_capture_output()` in [exercise_runtime_support/exercise_framework/runtime.py](../../exercise_runtime_support/exercise_framework/runtime.py) executes code by tag, so missing or misspelled tags will cause automated tests to fail.
