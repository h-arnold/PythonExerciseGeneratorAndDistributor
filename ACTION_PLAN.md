...
- [x] The phase is not complete until the `ex007` naming mismatch is resolved or explicitly treated as a prerequisite blocker for trusting the CLI `--variant <student|solution>` variant-selection contract. (Resolved via the Phase 4 audit recorded in `PHASE_4_MIGRATION_CHECKLIST.md`.)
...
- [x] Task 1 completed on `2026-03-17`: the shared grading resolver contract now runs through `exercise_runtime_support/exercise_framework/paths.py` and `exercise_runtime_support/notebook_grader.py`, rejects path-like strings plus source-repository `notebooks/` paths, and still anchors relative canonical `Path` inputs to the repository root.
- [x] Focused Phase 8 framework coverage now locks that contract in `tests/exercise_framework/test_paths.py`, `tests/exercise_framework/test_parity_paths.py`, and `tests/exercise_framework/test_runtime.py`, including exercise-key resolution, legacy path rejection, and relative canonical path anchoring.
...
- [x] Task 3 completed on `2026-03-17`: packaged/template integration coverage now uses the final variant-driven contract (`PYTUTOR_ACTIVE_VARIANT`) for student-mode checks, while existing selector/collector coverage and workflow consumer checks continue to pin canonical registry selection, exercise-directory collection, and `--variant <student|solution>` workflow usage across `scripts/template_repo_cli/core/selector.py`, `scripts/template_repo_cli/core/collector.py`, `tests/template_repo_cli`, `tests/exercise_runtime_support/test_consumer_matrix.py`, and the workflow YAML files.
...
- [x] Update [scripts/template_repo_cli/core/selector.py](scripts/template_repo_cli/core/selector.py) to select exercises from the new canonical tree.
- [x] Update [scripts/template_repo_cli/core/collector.py](scripts/template_repo_cli/core/collector.py) to collect files from each exercise directory.
